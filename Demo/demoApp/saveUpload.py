        ...
        if '_upload' in request.POST:
            ...
            if form.is_valid():
                ...
                if str(f.pc.name).endswith('.las'):
                    ...
                    #importance = random.randint(10000, size=10000)  #uniform distribution of random numbers between 0-10000 --> array
                    for i in range(10000):
                        if 'X' in dir(f[i]) and 'Y' in dir(f[i]) and 'Z' in dir(f[i]):
                            #p = Point_Cloud(x=f[i].X, y=f[i].Y, z=f[i].Z, m = importance[i], name = n)
